...
def train_model(data: pd.DataFrame) -> tuple[XGBClassifier, list[str]]:
    ...
    excluded = {"forecast_date", "cell_id", "cell_lat", "cell_lon", "grid_x", "grid_y", "has_m5_future", "n_m5_future", "max_m5_future"}
    ...
    if positive == 0 or negative == 0:
        raise RuntimeError(f"Clases desbalanceadas inválidas: Positivas={positive}, Negativas={negative}")
...
def generate_risk_map(model: XGBClassifier, data: pd.DataFrame, feature_columns: list[str]) -> pd.DataFrame:
    X = data[feature_columns].replace([np.inf, -np.inf], np.nan)
    probabilities = model.predict_proba(X)[:, 1]

    result = data[["forecast_date", "cell_id", "grid_x", "grid_y", "cell_lat", "cell_lon", "has_m5_future", "n_m5_future"]].copy() if "forecast_date" in data.columns else data[["cell_id", "grid_x", "grid_y", "cell_lat", "cell_lon", "has_m5_future", "n_m5_future"]].copy()
