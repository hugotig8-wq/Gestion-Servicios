"""Evaluation metrics module for spatial seismic forecasts."""
...
def calculate_top_capture(risk_map: pd.DataFrame) -> Dict[str, Any]:
    """Calculate ratio of events captured within highest-risk cells."""
    results = {}
    ...
    for percentage in [0.10, 0.20, 0.30, 0.50]:
        ...
        capture_rate = (
            (captured_events / total_events) if total_events > 0 else np.nan
        )
        results[f"top_{int(percentage * 100)}_percent"] = {
            ...
            "capture_rate": (
                float(capture_rate) if not np.isnan(capture_rate) else None
            ),
        }
...
def calculate_metrics(risk_map: pd.DataFrame) -> Dict[str, Any]:
    """Compute probabilistic and ranking evaluation metrics."""
