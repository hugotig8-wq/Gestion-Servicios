...
export function validaRegExpPassword (passw, confPassw){
        const regExpPassword = /^(?=[A-Za-z])(?=.*[0-9])(?=.*[A-Z])(?=.*[a-z])(?=.*[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?`~])[A-Za-z0-9!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?`~]{8,26}$/;
        const coincide = validaRegExpConfPassword(passw, confPassw)
        return {boolRegExp: regExpPassword.test(passw), coincide:coincide}
}
...
export const validadores = {
        ...
        password: (data)=>validaRegExpPassword(data.password, data.confPassword),
